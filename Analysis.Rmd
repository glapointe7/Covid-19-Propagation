...
$$
where $k \in \mathbb{R}$ is the parameter to provide. For example with $k = 2$, non-aberrant propagation accelerations in the Canada have to be exclusively between 2 standard deviations `r infected.acceleration.mean - 2*infected.acceleration.sd` and `r infected.acceleration.mean + 2*infected.acceleration.sd`.
...
$$
\begin{aligned}
  \frac{\partial R(t)}{\partial t} &= \gamma I_c(t) \\
  \frac{\partial D(t)}{\partial t} &= \alpha I_c(t).
\end{aligned}
$$
...
\begin{aligned}
  ...
  \frac{\partial I_a(t)}{\partial t} &= \beta S(t)I_c(t) - (\gamma + \alpha) I_c(t) \\
  \frac{\partial R(t)}{\partial t} &= \gamma I_c(t) \\
  \frac{\partial D(t)}{\partial t} &= \alpha I_c(t)
\end{aligned}
$$
...
We know that $\frac{\partial R(t)}{\partial t} = \gamma I_c(t)$. Replacing $I_c(t)$ by the equation we found and integrating on both sides from $0$ to $t$, we have to solve
$$
R(t) = \gamma N \int_0^t \frac{1}{1 - \left(1 - \frac{N}{I_c(0)}\right)e^{-N \beta x}} dx.
$$
Equivalently, by multiplying by $1 = \frac{\left(\frac{I_c(0)}{I_c(0) - N}\right)e^{N \beta x}}{\left(\frac{I_c(0)}{I_c(0) - N}\right)e^{N \beta x}}$, we get
$$
R(t) = \gamma N \int_0^t \frac{\left(\frac{I_c(0)}{I_c(0) - N}\right) e^{N \beta x}}{\left(\frac{I_c(0)}{I_c(0) - N}\right) e^{N \beta x} - 1} dx.
$$
Let $u(x) = \left(\frac{I_c(0)}{I_c(0) - N}\right) e^{N \beta x}$. Then, $du = N \beta \left(\frac{I_c(0)}{I_c(0) - N}\right) e^{N \beta x} dx$ or equivalently $\frac{du}{N \beta} = u(x)dx$. Replacing in $R(t)$ and solving give:
$$
\begin{aligned}
  R(t) &= \gamma N \int_0^t \frac{du}{N \beta (u - 1)} \\
       &= \frac{\gamma}{\beta} \int_0^t \frac{du}{u - 1} \\
       &= \frac{\gamma}{\beta} (\ln |u(t) - 1| - \ln |u(0) - 1|)
\end{aligned}
$$
However, $u(0) =\left(\frac{I_c(0)}{I_c(0) - N}\right) e^{N \beta 0} = \left(\frac{I_c(0)}{I_c(0) - N}\right)$. We also remove the absolute values because the value evaluated in the logarithm functions are greater than 0. This gives:
$$
R(t) = \frac{\gamma}{\beta} \left(\ln (u(t) - 1) - \ln \left(\frac{I_c(0)}{I_c(0) - N}\right) \right)
$$
Replacing $u(t)$ by $\left(\frac{I_c(0)}{I_c(0) - N}\right) e^{N \beta t}$ in the equation gives (including simplifications):
$$
\begin{aligned}
  R(t) &= \frac{\gamma}{\beta} \left(\ln \left(\left(\frac{I_c(0)}{I_c(0) - N}\right) e^{N \beta t} - 1\right) - \ln \left(\frac{I_c(0)}{I_c(0) - N}\right) \right) \\
       &= \frac{\gamma}{\beta} \ln \left(e^{N \beta t} + \frac{N}{I_c(0)} - 1\right)
\end{aligned}
$$
...
$$
D(t) = \frac{\alpha}{\beta} \ln \left(e^{N \beta t} + \frac{N}{I_c(0)} - 1\right).
$$
